...
一个浏览器脚本，帮你在DGUT莞工【求是读书计划】里**自动计时 + 自动翻页**，无需手动盯着屏幕翻书，就能完成每本书 4 小时的学分要求。

## 它能做什么

- **自动翻页**：打开课件后自动一页页往后翻，到最后一页若时长未满会回到开头继续循环
...
**第一步**：给你的浏览器装 Tampermonkey/脚本猫 的扩展。

- 如果你用的是 Chrome、Edge 或 Firefox
  - 选择1：[安装 Tampermonkey](https://www.tampermonkey.net/)
  - 选择2：[安装 脚本猫](https://scriptcat.org/)
- 如果你用的是国产浏览器（360、QQ、搜狗等），一般在浏览器的"扩展商店"里搜索"Tampermonkey"、"油猴"或“脚本猫”就能找到

**第二步**：安装此脚本。

- 方式一：打开 [Greasy Fork 页面](https://greasyfork.org/zh-CN/scripts/577055)，点"安装此脚本"
- 方式二：打开 [ScriptCat 脚本猫](https://scriptcat.org/zh-CN/script-show-page/6260)，点“安装脚本”
- 方式三：下载本仓库里的 `DGUT求是读书-全自动阅读助手.js` 文件，打开 Tampermonkey → 添加新脚本 → 把文件内容全部复制进去 → 保存
...
1. **换书看是没问题的**：在课程里点击其他书目，脚本会自动保存当前书的进度，开始给新书计时
2. **读完一本书会自动换下一本**，不需要手动操作
3. **每本书需要读 4 小时以上**才算完成学分，脚本会帮你读够时间
4. **一些旧版浏览器可能不支持**：如果发现面板没出来，建议换成 Edge 或 Chrome 浏览器试试
